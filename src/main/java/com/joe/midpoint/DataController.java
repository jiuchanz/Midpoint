...
public class DataController {
  ...
  private void clearOutdatedEntries() {
    Map<Integer, Update> newDataUpdates =
        dataUpdates.entrySet().stream()
            .filter(
                entry ->
                    entry.getValue().getCreationDate().getTime() + expireTime
                        > new Date().getTime())
            .collect(Collectors.toMap(entry -> entry.getKey(), entry -> entry.getValue()));
    if (dataUpdates.size() != newDataUpdates.size()) {
      logger.info(
          "removed" + String.valueOf(dataUpdates.size() - newDataUpdates.size()) + " entries");
    }
    dataUpdates = newDataUpdates;
    Map<Integer, Update> newCommandUpdates =
        commandUpdates.entrySet().stream()
            .filter(
                entry ->
                    entry.getValue().getCreationDate().getTime() + expireTime
                        > new Date().getTime())
    ...
    if (commandUpdates.size() != newCommandUpdates.size()) {
      logger.info(
          "removed"
              + String.valueOf(commandUpdates.size() - newCommandUpdates.size())
              + " entries");
    }
